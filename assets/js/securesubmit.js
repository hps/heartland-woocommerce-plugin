jQuery( function() {
    ...
    if (typeof $form.data('events').submit !== 'undefined' && $form.data('events').submit.length > 0) {
        var length = $form.data('events').submit.length;
        for (var i = 0; i < length; i++) {
            originalHandlers.push($form.data('events').submit[i].handler);
        }
        $form.unbind('submit');
    }

    $form.on('checkout_place_order_securesubmit', function( event ) {
        return secureSubmitFormHandler(originalHandlers);
    });

    jQuery('form.checkout, form#order_review').submit(function(){
        return secureSubmitFormHandler(originalHandlers);
    });
...
function secureSubmitFormHandler(originalHandlers) {
        ...
        if (jQuery( 'input.securesubmit_token' ).size() == 0) {
            ...
            hps.tokenize({
                ...
                success: function(response) {
                    secureSubmitResponseHandler(response, originalHandlers);
                },
...
function secureSubmitResponseHandler( response, originalHandlers ) {
    ...
    if ( response.message ) {
        ...
        if (typeof originalHandlers !== 'undefined' && originalHandlers !== []) {
            var length = originalHandlers.length;
            for (var i = 0; i < length; i++) {
                $form.submit(originalHandlers[i]);
            }
        }
