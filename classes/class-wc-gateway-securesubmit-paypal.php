...
class WC_Gateway_SecureSubmit_PayPal extends WC_Payment_Gateway {
    ...
    public function process_payment( $order_id ) {
        ...
        $buyer->returnUrl = add_query_arg( 'pp_action', 'revieworder', get_permalink( wc_get_page_id( 'review_order' )));      
        $buyer->cancelUrl = WC()->cart->get_cart_url();      
        ...
        //$this->log('subtotal: ' . $order->get_subtotal() . '; shippingAmount: ' . $order->get_total_shipping() . 
        //                                '; shippingAmount: ' . $order->get_total_shipping() . ' ; taxAmount: ' . $order->get_total_tax());
        $lineItems = $this->getLineItems($order);
        
        $orderTotal = $order->order_total;
        $currency = strtolower(get_woocommerce_currency());      
                
        $errorReturnArray = array('result'   => 'failure');
    ...
    public function process_refund( $order_id, $amount = null, $reason = '' ) {
        ...
        //WC_Gateway_Paypal_SecureSubmit_Refund::$api_username  = $this->get_option( 'api_username' );
        //WC_Gateway_Paypal_SecureSubmit_Refund::$api_password  = $this->get_option( 'api_password' );
        //WC_Gateway_Paypal_SecureSubmit_Refund::$api_signature = $this->get_option( 'api_signature' );

        //$result = WC_Gateway_Paypal_SecureSubmit_Refund::refund_order( $order, $amount, $reason, $this->testmode );
    ...
    {          
        ...
        $orderTotal = number_format($payment->subtotal + $payment->shippingAmount + $payment->taxAmount);
        $currency = strtolower(get_woocommerce_currency());
