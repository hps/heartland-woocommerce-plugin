...
class WC_Gateway_SecureSubmit_PayPal extends WC_Payment_Gateway
{
    ...
    public function admin_options()
    {
        if ($this->is_valid_for_use()) {
            ...
            ?>
            <div class="inline error"><p><strong><?php _e('Gateway Disabled', 'woocommerce'); ?></strong>:
            <?php _e('Heartland PayPal does not support your store currency.', 'woocommerce'); ?></p></div>
            <?php
        }
    }
    ...
    public function process_paypal_express_payment_checkout()
    {
        ...
        try {
            ...
            self::debugLog('Portico session creation failedException redirect url = ' . $redirectUrl);
        ...
        echo('<script type="application/javascript">window.location.href="'.$redirectUrl.'";</script>');
        self::debugLog('$redirectUrl = ' . $redirectUrl);
        self::debugLog('End function : ' . __FUNCTION__);
        self::debugLog(' ------------------------------------------------- [ SENDING TO PAYPAL ]');
        exit();
    }
    ...
    {
        self::debugLog('Begin function : ' . __FUNCTION__);
        self::debugLog('$order_id = ' . $order_id);
        ...
        if (isset($override) && $override==1) {
            ...
            self::debugLog('process_payment quick exit process payment');
            self::debugLog('End function [a]: ' . __FUNCTION__);
            return;
        }
        $woocommerce = WC();
        $this->set_session('checkout_form', $_POST);
        ...
        self::debugLog('$isExpressCheckout = ' . $isExpressCheckout);
        if (!isset($expressCheckout) || $isExpressCheckout == false) {
            ...
            $response;
            try {
                $response = $porticoService->createSession(
                    $orderTotal,
                    $currency,
                    $buyer,
                    $payment,
            ...
            $redirectUrl = $response->redirectUrl;
            self::debugLog('Process_Payment normal exit to url : ' . $redirectUrl);
            self::debugLog('End function [b]: ' . __FUNCTION__);
            return array(
                'result' => 'success',
                'redirect' => $redirectUrl
            );
            self::debugLog('End function [c]: ' . __FUNCTION__);
        } else {
    ...
    public function process_refund($order_id, $amount = null, $reason = '')
    {
        ...
        if (! $this->can_refund_order($order)) {
            self::debugLog('Refund Failed: No transaction ID');
            return false;
        }
        $response = null;
        try {
            ...
            self::debugLog('process_refund : $error = ' . $error);
            if (function_exists('wc_add_notice')) {
                wc_add_notice($error, 'error');
            } else {
                $woocommerce->add_error($error);
        ...
        self::debugLog('process_refund : $response = ' . print_r($response, true));
        ...
        self::debugLog('Refund Failed in Portico call with responseCode ' . $response->responseCode);
        return false;
    }
    ...
    {
        self::debugLog('Begin function : ' . __FUNCTION__);
        self::debugLog('$_GET = ' . print_r($_GET, true));
        ...
        self::debugLog('Exiting function : ' . __FUNCTION__);
    }
    ...
    private function paypal_review_order()
    {
        ...
        if (isset($_GET['token'])) {
            self::debugLog('Setting session TOKEN value to ' . $_GET['token']);
            $token = $_GET['token'];
        ...
        if (!empty($porticoSessionInfo)) {
            ...
            self::debugLog("...ERROR: GetShippingDetails returned empty result");
        }
    ...
    */
    private function paypal_create_order()
    {
        ...
        self::debugLog('$chosen_shipping_methods = ' . print_r($chosen_shipping_methods, true));
        ...
        $_POST['billing_address_2'] = isset($hpsShippingInfo->address->address_1) ? $hpsShippingInfo->address->address_1 : '';
        ...
        $_POST['shipping_address_2'] = isset($hpsShippingInfo->address->address_1) ? $hpsShippingInfo->address->address_1 : '';
        ...
        self::debugLog('paypal_create_order() : $_POST = ' . print_r($_POST, true));
        $this->set_session('ppexpress_checkout_form', serialize($_POST));
        $all_notices = WC()->session->get('wc_notices', array());
        if(sizeof($all_notices)>0) {
            self::debugLog('$all_notices = ' . print_r($all_notices, true));
        }
    ...
    private function paypal_finalize_order()
    {
        ...
        if (!isset($order_id)) {
            self::debugLog('No order id found, creating order');
            $this->paypal_create_order();
            $order_id = WC()->session->order_awaiting_payment;
        }
        self::debugLog('paypal_finalize_order : $order_id = ' . $order_id);
        ...
        self::debugLog('$billingAddress = ' . print_r($billingAddress, true));
        self::debugLog('$shippingAddress = ' . print_r($shippingAddress, true));
        ...
        self::debugLog('paypal_finalize_order() : $order = ' . print_r($order, true));
        ...
        self::debugLog('$response->responseCode = ' . $response->responseCode);
        if ($response->responseCode == '0') {
            ...
            self::debugLog('Redirecting to ' . $this->get_return_url($order));
            self::debugLog('End function : ' . __FUNCTION__);
            wp_redirect($this->get_return_url($order));
            exit();
        }
        self::debugLog('Error Section of  : ' . __FUNCTION__);
        wc_add_notice(sprintf(__('There was a problem paying with PayPal.  Please try another method.', 'wc_securesubmit')), 'error');
        self::debugLog('Order did not complete successfully. Order ID: ' . $order_id . '. Return Status Code: ' . $response->responseCode);
        wp_redirect(get_permalink(wc_get_page_id('cart')));
        self::debugLog('End function : ' . __FUNCTION__);
        exit();
    }
    ...
    {
        self::debugLog('Saved to session : ' . print_r($key, true) . ' = ' . print_r($value, true));
        WC()->session->$key = $value;
    }

    private function get_session($key)
    {
        self::debugLog('Requested session value : ' . print_r($key, true));
